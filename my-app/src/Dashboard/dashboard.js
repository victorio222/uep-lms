...
const Dashboard = () => {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const courses = response.data.map((course) => ({
                ...course,
                status: course.status || "unknown", // Default value for status
            }));
            setSubjects(courses);
            setFilteredSubjects(courses); // Initialize filtered list
        } catch (error) {
            console.error("Error fetching courses:", error);
        }
    };
    ...
    const courseStatusDisplay = (courseStatus) => {
        if (!courseStatus) {
            return "Unknown Status"; // Fallback for undefined status
        }
        if (courseStatus.toLowerCase() === "published") {
            return "Unpublish";
        } else {
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-6">
                        {filteredSubjects.length > 0 ? (
                            filteredSubjects.map((subject) => (
                                <div
                                    key={subject.courseId}
                                    className={`group relative bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-all duration-300 ease-in-out ${
                                        subject.status === "unpublished"
